const { query } = require("../configuration/db");
    ...
    if (min && !max && value.length < min) {
      this.setError(
        ...
        this.customMessages?.[key]?.length ||
          `Please enter ${min} character min`
      );
    } else if (!min && max && value.length > max) {
      this.setError(
        key,
        this.customMessages?.[key]?.length ||
          `You can not enter more than ${max} character`
      );
    } else if (min && max && (value.length < min || value.length > max)) {
      this.setError(
        key,
        this.customMessages?.[key]?.length ||
          `This field value must be between ${min} and ${max}`
      );
    }
  }
    ...
    if (this.data[params] !== value) {
      this.setError(
        ...
        this.customMessages?.[key]?.match ||
          `The value does not much ${params}`
      );
    }
  }
    ...
    if (!validUsername || value?.length < 2) {
      this.setError(
        ...
        this.customMessages?.[key]?.username ||
          "Incorrect user name (letters, numbers, dot or underscore)"
      );
    }
  }
      ...
      if (!row) {
        this.setError(
          ...
          this.customMessages?.[key]?.exists ||
            `The row does not exist on the table ${tableName}`
        );
      }
    } catch (error) {
      throw error;
      ...
      if (row) {
        this.setError(
          ...
          this.customMessages?.[key]?.unique ||
            `The ${columnName} must be unique ${tableName}`
        );
      }
    } catch (error) {
      throw error;
